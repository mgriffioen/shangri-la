...
function applyCanvasTransform() {
  ...
  canvas.style.transform = `translate(${pinch.panX}px, ${pinch.panY}px) scale(${pinch.zoom})`;
}
...
canvas.addEventListener('touchstart', (e) => {
  if (e.touches.length === 2) {
    e.preventDefault();
    pinch.active   = true;
    pinch.lastDist = pinchDist(e.touches[0], e.touches[1]);
  }
}, { passive: false });
...
canvas.addEventListener('touchmove', (e) => {
  if (e.touches.length === 2 && pinch.active) {
    ...
    const scale    = newDist / pinch.lastDist;
    const newZoom  = Math.max(0.5, Math.min(8, pinch.zoom * scale));
    const factor   = newZoom / pinch.zoom;
    ...
    pinch.panX  = midX - (midX - pinch.panX) * factor;
    pinch.panY  = midY - (midY - pinch.panY) * factor;
    pinch.zoom  = newZoom;
...
canvas.addEventListener('touchend', (e) => {
  if (e.touches.length === 0 && e.changedTouches.length === 1) {
    const now = Date.now();
    if (now - lastTap < 300) {
      pinch.zoom = 1; pinch.panX = 0; pinch.panY = 0;
      applyCanvasTransform();
    }
    lastTap = now;
  }
